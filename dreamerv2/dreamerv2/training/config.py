...
class MiniWoBConfig():
    ...
    embedding_size: int = 200
    rssm_node_size: int = 200
    nl_embedding_size: int = 384
    rssm_info: Dict = field(default_factory=lambda:{'deter_size':200, 'stoch_size':256, 'class_size':20, 'category_size':20, 'min_std':0.1})
    ...
    horizon: int = 10
    lr: Dict = field(default_factory=lambda:{'model':2e-4, 'actor':4e-5, 'critic':1e-4})
    loss_scale: Dict = field(default_factory=lambda:{'kl':0.1, 'reward':1.0, 'discount':1.0})
    kl: Dict = field(default_factory=lambda:{'use_kl_balance':True, 'kl_balance_scale':0.8, 'use_free_nats':True, 'free_nats':0.1})
    use_slow_target: float = True
    slow_target_update: int = 100
    slow_target_fraction: float = 1.00
    ...
    #actor critic
    actor: Tuple = (
        field(default_factory=lambda:{'layers':3, 'node_size':300, 'dist':'category', 'min_std':1e-4, 'init_std':5, 'mean_scale':5, 'activation':nn.ELU}),
        field(default_factory=lambda:{'layers':3, 'node_size':300, 'dist':'category', 'min_std':1e-4, 'init_std':5, 'mean_scale':5, 'activation':nn.ELU}),
        field(default_factory=lambda:{'layers':3, 'node_size':300, 'dist':'category', 'min_std':1e-4, 'init_std':5, 'mean_scale':5, 'activation':nn.ELU})
    )

    critic: Dict = field(default_factory=lambda:{'layers':3, 'node_size':300, 'dist': 'normal', 'activation':nn.ELU})
    expl: Dict = field(default_factory=lambda:{'train_noise':0.4, 'eval_noise':0.0, 'expl_min':0.05, 'expl_decay':500.0, 'expl_type':'epsilon_greedy'})
    actor_grad: str ='reinforce'
    actor_grad_mix: int = 0.0
    actor_entropy_scale: float = 1e-3
    ...
    obs_encoder: Dict = field(default_factory=lambda:{'layers':3, 'node_size':100, 'dist': None, 'activation':nn.ELU, 'kernel':3, 'depth':16})
    obs_decoder: Dict = field(default_factory=lambda:{'layers':3, 'node_size':100, 'dist':'normal', 'activation':nn.ELU, 'kernel':3, 'depth':16})
    reward: Dict = field(default_factory=lambda:{'layers':3, 'node_size':300, 'dist':'normal', 'activation':nn.ELU})
    discount: Dict = field(default_factory=lambda:{'layers':3, 'node_size':100, 'dist':'binary', 'activation':nn.ELU, 'use':True})
